...
class Student {
    ...
    fees;
    feePaid;
    constructor(studentName, studentClass, studentRollNo, studentCourses) {
...
const addStudent = async () => {
    ...
    if (studentInfo.confirmAddingStudent) {
        ...
        let student = new Student(studentInfo.studentName, parseInt(studentInfo.studentClass), studentInfo.studentRollNo, studentInfo.studentEnrolledCourses);
...
const enrollStudent = async () => {
    ...
    notEnrolledCourse = courses.filter((e) => !students[Object.keys(students)[studentsNames.indexOf(student.student)]].courses.includes(e));
    ...
    if (course.confirmEnrollingStudent) {
        students[Object.keys(students)[studentsNames.indexOf(student.student)]].courses = students[Object.keys(students)[studentsNames.indexOf(student.student)]].courses.concat(course.courses);
        students[Object.keys(students)[studentsNames.indexOf(student.student)]].fees =
            students[Object.keys(students)[studentsNames.indexOf(student.student)]]
                .courses.length * 100;
        students[Object.keys(students)[studentsNames.indexOf(student.student)]].feePaid = false;
        spinner.success({
            text: `${student.student} enrolled to ${course.courses.join(" and ")} course successfully\n`,
        });
    }
    else {
        spinner.error({
...
const payTuitionFees = async () => {
    ...
    let feePaidFalseStudentsIds = Object.keys(students).filter((e) => !students[e].feePaid);
    let feePaidFalseStudentsNames = feePaidFalseStudentsIds.map((e) => students[e].name);
    let studentsNames = Object.keys(students).map((e) => students[e].name);
    if (Object.keys(students).length == 0) {
        spinner.error({ text: "No students found\n" });
        return balance;
    ...
    spinner.start();
    await sleep();
    students[Object.keys(students)[studentsNames.indexOf(payFeesStudent.student)]].feePaid = true;
    balance +=
        students[Object.keys(students)[studentsNames.indexOf(payFeesStudent.student)]].fees;
    spinner.success({
